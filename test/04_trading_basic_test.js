...
contract('Simple Order Tests', accounts => {
    before(async () => {
        ...
        await ExchangeInstance.addToken('HIMELBRAND', instanceToken.address)
        await instanceToken.approve(ExchangeInstance.address, 2000)
        const txResponse = await ExchangeInstance.depositToken('HIMELBRAND', 2000)
    ...
    it('should be possible to add a limit buy order', async () => {
        ...
        let orderBook = await myExchangeInstance.getBuyOrderBook.call('HIMELBRAND')
        ...
        const txResult = await myExchangeInstance.buyToken('HIMELBRAND', web3.toWei(1, 'finney'), 5)
        ...
        orderBook = await myExchangeInstance.getBuyOrderBook.call('HIMELBRAND')
    ...
    it('should be possible to add three limit buy orders', async () => {
        ...
        let txResult = await myExchangeInstance.buyToken('HIMELBRAND', web3.toWei(2, 'finney'), 5)
        assert.equal(txResult.logs[0].event, 'LimitBuyOrderCreated', 'The Log-Event should be LimitBuyOrderCreated')
        txResult = await myExchangeInstance.buyToken('HIMELBRAND', web3.toWei(1.4, 'finney'), 5)
        assert.equal(txResult.logs[0].event, 'LimitBuyOrderCreated', 'The Log-Event should be LimitBuyOrderCreated')
        orderBook = await myExchangeInstance.getBuyOrderBook.call('HIMELBRAND')
    ...
    it('should be possible to add two limit sell orders', async () => {
        const myExchangeInstance = await Exchange.deployed()
        let orderBook = await myExchangeInstance.getSellOrderBook.call('HIMELBRAND')
        let txResult = await myExchangeInstance.sellToken('HIMELBRAND', web3.toWei(3, 'finney'), 5)
        assert.equal(txResult.logs.length, 1, 'There should have been one Log Message emitted.')
        assert.equal(txResult.logs[0].event, 'LimitSellOrderCreated', 'The Log-Event should be LimitSellOrderCreated')
        txResult = await myExchangeInstance.sellToken('HIMELBRAND', web3.toWei(6, 'finney'), 5)
        orderBook = await myExchangeInstance.getSellOrderBook.call('HIMELBRAND')
    ...
    it('should be possible to create and cancel a buy order', async () => {
        ...
        let orderBook = await myExchangeInstance.getBuyOrderBook.call('HIMELBRAND')
        const orderBookLengthBeforeBuy = orderBook[0].length
        let txResult = await myExchangeInstance.buyToken('HIMELBRAND', web3.toWei(2.2, 'finney'), 5)
        assert.equal(txResult.logs.length, 1, 'There should have been one Log Message emitted.')
        assert.equal(txResult.logs[0].event, 'LimitBuyOrderCreated', 'The Log-Event should be LimitBuyOrderCreated')
        ...
        txResult = await myExchangeInstance.cancelOrder('HIMELBRAND', false, web3.toWei(2, 'finney'), orderKey)
    ...
    it('should be possible to create and cancel a sell order', async () => {
        ...
        let txResult = await myExchangeInstance.sellToken('HIMELBRAND', web3.toWei(2.5, 'finney'), 5)
        assert.equal(txResult.logs.length, 1, 'There should have been one Log Message emitted.')
        assert.equal(txResult.logs[0].event, 'LimitSellOrderCreated', 'The Log-Event should be LimitSellOrderCreated')
        const orderKey = txResult.logs[0].args._orderKey
        orderBook = await myExchangeInstance.getSellOrderBook.call('HIMELBRAND')
        ...
        txResult = await myExchangeInstance.cancelOrder('HIMELBRAND', true, web3.toWei(2.5, 'finney'), orderKey)
